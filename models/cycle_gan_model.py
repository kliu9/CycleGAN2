...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG1, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.netG2, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define discriminators
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf, opt.netD1,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD2,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    # only for training discriminator
    ...
    def save(self):
        torch.save(self.netD_A.state_dict(), "netd_weights_A.pth")

    def get_current_D_losses(self):
        """Return traning losses / errors. train.py will print out these errors on console, and save them to a file"""
        # float(...) works for both scalar tensor and float number
        return float(getattr(self, 'loss_D_A'))
